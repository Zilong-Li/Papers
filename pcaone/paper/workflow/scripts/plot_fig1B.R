...
pops <- read.table(snakemake@config[["asian"]]$fam, header = F)[, 1:2]
coljco <- c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF", "#7AA6DCFF", "#003C67FF", "#8F7700FF", "#3B3B3BFF", "#A73030FF", "#4A6990FF")
pctruth <- function() {
  plot_pca_asia(truth[, 1], -truth[, 2], coljco[as.numeric(factor(pops[, 2]))], "Full SVD", bs = 0.1, xlab = "")
  legend("topleft", legend = sort(unique(pops[, 2])), col = coljco[as.numeric(factor(sort(unique(pops[, 2]))))], pch = 19, bty = "n", cex = 0.8, y.intersp = 0.8, inset = c(0, -0.03))
}
pcpcaone <- function() {
  plot_pca_asia(-l.pc$pcaone.f[, 1], -l.pc$pcaone.f[, 2], coljco[as.numeric(factor(pops[, 2]))], "PCAone", bs = 0.1, xlab = "")
}
pcflashpca <- function() {
  plot_pca_asia(-l.pc$flashpca[, 1], l.pc$flashpca[, 2], coljco[as.numeric(factor(pops[, 2]))], "FlashPCA2", bs = 0.1, xlab = "")
}
pcfastpca <- function() {
  plot_pca_asia(l.pc$fastpca[, 1], -l.pc$fastpca[, 2], coljco[as.numeric(factor(pops[, 2]))], "FastPCA", bs = 0.1, xlab = "")
}
pcplink2 <- function() {
  plot_pca_asia(-l.pc$plink2[, 1], -l.pc$plink2[, 2], coljco[as.numeric(factor(pops[, 2]))], "Plink2", bs = 0.1, xlab = "")
}
pcpropca <- function() {
  plot_pca_asia(l.pc$propca[, 1], l.pc$propca[, 2], coljco[as.numeric(factor(pops[, 2]))], "ProPCA", xlab = "PC1")
}
pcterapca <- function() {
  plot_pca_asia(-l.pc$terapca[, 1], l.pc$terapca[, 2], coljco[as.numeric(factor(pops[, 2]))], "TeraPCA", xlab = "PC1")
}
a0 <- function() {
  par(mar = c(0, 0, 0, 0))
