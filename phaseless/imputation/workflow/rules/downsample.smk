...
rule downsample_bam:
    output:
        os.path.join(OUTDIR, "downsample", "{sample}_{depth}x_{chrom}.bam"),
    params:
        N="downsample_bam",
        depth=lambda wildcards: SAMPLES[wildcards.sample]["depth"],
        bam=lambda wildcards: SAMPLES[wildcards.sample]["bam"],
    log:
        os.path.join(OUTDIR, "downsample", "{sample}_{depth}x_{chrom}.bam.llog"),
    threads: 1
    conda:
        "../envs/pandas.yaml"
    shell:
...
rule bamlist:
    ...
    output:
        os.path.join(OUTDIR, "downsample", "{depth}x_{chrom}.bamlist"),
    log:
        os.path.join(OUTDIR, "downsample", "{depth}x_{chrom}.bamlist.llog"),
    params:
        N="bamlist",
    conda:
        "../envs/pandas.yaml"
