...
rule get_pos_from_refpanel:
    ...
    shell:
        """
        bcftools view -v snps -m2 -M2 -g het --threads 4 {params.vcf}| bcftools norm - -d snps | bcftools view -G {output.vcf} -Oz -o {output.sites} --threads 4 && tabix -f {output.sites} && \
